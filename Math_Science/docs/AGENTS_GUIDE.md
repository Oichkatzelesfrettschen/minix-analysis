...
Utilize the `Makefile` for common tasks. For Windows users without WSL/Git Bash, PowerShell scripts are often available or direct Python script execution is possible.
...
*   **ASCII-only Policy**: Strict ASCII-only for code and documentation (`.py`, `.md`, `.tex`, `.yml`, `.ps1`, `.sh`). Use LaTeX macros or ASCII transliterations for special characters. Run `make ascii_guard` locally. Data dumps in `data/`, `extracted_data/`, `source_materials/` are exceptions.
...
*   **Pre-commit hooks**: Install locally (`pip install pre-commit`, `pre-commit install`) to enforce ASCII and quick tests.
...
*   Keep PowerShell execution policy at least `RemoteSigned` for automation scripts.
...
*   **Process**: Add opening story, worked examples, physical interpretations, framework attribution (`\aetherattr{}`, etc.), modularize equations, test compile, and **CRITICALLY verify agent output with bash commands** (`wc -l`, `ls -lh`, `grep`).

### 1.8 Running Agents in Parallel

*   **Recommended**: Use 2-3 agents maximum for parallel work.
*   **Verification Protocol (CRITICAL)**: NEVER trust agent claims without verification. Use `wc -l`, `ls -lh`, `grep`, `head` to verify files changed (line counts, timestamps, content spot-checks). Previous sessions showed agents claiming success but files unchanged; strict verification prevents this.
...
*   **Validation**: Individual chapter compilation, cross-reference resolution, bibliography citation resolution, catalog-module parity (`catalog_parity.py`), test suite pass (`test_compilation.ps1`).
...
*   **Cross-Platform Compatibility**: Explicitly addresses and documents Windows compatibility for the build system (e.g., `bash` environment setup for `make` commands).
*   **Error Handling in `Makefile`**: `|| true` has been removed from critical `Makefile` targets (`validate`, `test`, `latex`, `latex_strict`, `ascii_guard`) to ensure explicit failure on error.
*   **CI Implementation**: Basic GitHub Actions workflow (`.github/workflows/main.yml`) implemented to run `make ci` on pull requests and `make latex_strict` on `main` branch pushes. Linting (`flake8`) and type checking (`mypy`) steps added.
...
*   **"Memory dump file not found" / "pdflatex.fmt not found"**: MiKTeX format files corrupted. Fix by updating formats in MiKTeX Console or via `initexmf` commands.
...
*   **"Module not found"**: Verify Python 3.10+ installed. Ensure `--base-dir` points to repository root. Use absolute paths or correct working directory.
