...
% Load modular style packages
\usepackage{minix-arxiv}   % ArXiv compliance (includes hyperref, colors, listings)
\usepackage{tikz}
\usepackage{pgfplots}
\usepackage{minix-styles}  % Diagram styles (imports minix-colors)

% Set PDF metadata (minix-arxiv helper)
\setpdfmetadata{%
    MINIX-3 CPU Interface Analysis: System Calls, Memory Management, and Performance%
}{%
    Oaich (eirikr)%
}{%
    Operating Systems, CPU Architecture, i386 Analysis%
}{%
    MINIX, i386, system calls, SYSENTER, SYSCALL, paging, TLB, performance%
}
