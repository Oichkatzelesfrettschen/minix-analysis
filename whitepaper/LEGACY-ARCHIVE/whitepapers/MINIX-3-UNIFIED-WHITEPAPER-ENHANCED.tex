...
\begin{figure*}[t]
\centering
\includegraphics[width=\textwidth]{../diagrams/tikz-generated/boot-sequence-data.pdf}
\caption{Boot Sequence Timeline: From bootloader handoff through 5 phases to userspace scheduler. The directed acyclic graph structure ensures deterministic initialization with no cyclic dependencies or deadlock risk.}
...
\caption{System Call Mechanisms: INT 80h (legacy), SYSENTER/SYSEXIT (Intel), SYSCALL/SYSRET (AMD). Each mechanism executes CPU microcode before kernel code receives control, with 3-4× performance differences.}
...
\begin{figure*}[h!]
\centering
\includegraphics[width=\textwidth]{../diagrams/tikz-generated/memory-regions.pdf}
\caption{Virtual Memory Layout: 4GB address space divided into user-space (0x00000000-0x7fffffff) and kernel-space (0x80000000-0xffffffff) regions. Page tables enable per-process isolation.}
...
\begin{figure*}[h!]
\centering
\includegraphics[width=\textwidth]{../diagrams/tikz-generated/ipc-architecture.pdf}
\caption{IPC Architecture: Message-passing infrastructure enabling user-space servers to communicate with kernel and each other. Isolation enforced via page tables; kernel mediates all IPC.}
\label{fig:ipc-arch}
\end{figure*}
...
% ============================================================================
% PART V: PROCESS MANAGEMENT & SCHEDULING
% ============================================================================
...
MINIX processes transition through multiple states during their lifetime:

\begin{figure*}[h!]
\centering
\includegraphics[width=\textwidth]{../diagrams/tikz-generated/process-states.pdf}
\caption{Process State Machine: Processes transition between RUNNABLE, BLOCKED (waiting for IPC or I/O), SLEEPING, and EXITED states. The scheduler maintains queues per priority level.}
\label{fig:process-states}
\end{figure*}

Key states:
\begin{itemize}
\item \textbf{RUNNABLE}: Process ready to execute; in scheduler queue
\item \textbf{BLOCKED}: Waiting for IPC response; held on message endpoint
\item \textbf{SLEEPING}: Dormant; waiting for signal or external event
\item \textbf{EXITED}: Terminated; awaiting parent reap
\end{itemize}

\subsection{Scheduling Algorithm}

MINIX uses priority-based preemptive scheduling:
\begin{itemize}
\item \textbf{Priority Levels}: 16 (0=highest, 15=lowest)
\item \textbf{Time Quantum}: 50ms per runnable process
\item \textbf{Preemption}: Higher-priority process immediately preempts lower
\item \textbf{Idle Process}: Always runnable; lowest priority fallback
\end{itemize}

% ============================================================================
% PART VI: PERFORMANCE ANALYSIS & OPTIMIZATION
% ============================================================================
...
% ============================================================================
% PART VII: FORMAL VERIFICATION & PROPERTIES
% ============================================================================
...
\begin{table}[h!]
\centering
\caption{MINIX 3 Kernel Metrics}
...
\midrule
Kernel Files & 91 & Source and header files \\
Kernel Lines & 19,659 & Lines of C code \\
System Calls & 38 & Microkernel interface \\
User-Space Servers & 11 & Filesystem, network, etc. \\
Device Drivers & 17 & I/O drivers \\
Boot Time & 85-100ms & From bootloader to userspace \\
Boot Functions & 121 & Across 8 files \\
\bottomrule
\end{tabular}
\end{table}
...
\item MINIX 3.4.0-RC6 Source Code (commit d5e4fc0)
\item IEEE 1003.1 (POSIX) Specification
\item Tanenbaum, A. S., et al. \textit{Structured Computer Organization} (6th ed.)
\end{itemize}
